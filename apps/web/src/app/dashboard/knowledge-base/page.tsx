import { Suspense } from "react";
import { FileText } from "lucide-react";
import { requireOrg } from "@/lib/auth";
import { PageHeader } from "@/components/layout/PageHeader";
import { MetricCard } from "@/components/dashboard/MetricCard";
import { KnowledgeStatusBadge } from "@/components/dashboard/StatusBadge";
import { UploadDocumentForm } from "@/components/knowledge/UploadDocumentForm";
import { CreateFaqForm } from "@/components/knowledge/CreateFaqForm";
import { GenerateKnowledgePanel } from "@/components/knowledge/GenerateKnowledgePanel";
import { KnowledgeAgentFilter } from "@/components/knowledge/KnowledgeAgentFilter";
import { listAiEmployees } from "@/modules/agents/data";
import {
  getKnowledgeMetrics,
  listFaqs,
  listKnowledgeDocuments,
} from "@/modules/knowledge/data";
import type { KnowledgeAgentFilter as AgentFilter } from "@/modules/knowledge/types";
import { formatDate } from "@/lib/formatting/datetime";

export const dynamic = "force-dynamic";
export const metadata = { title: "Knowledge Base" };

function assignedLabel(agentId: string | null, agentName?: string | null) {
  if (!agentId) return "All agents";
  return agentName?.trim() || "Assigned agent";
}

export default async function KnowledgeBasePage({
  searchParams,
}: {
  searchParams: Promise<{ agent?: string }>;
}) {
  const { organization } = await requireOrg();
  const params = await searchParams;
  const raw = params.agent?.trim();
  const agentFilter: AgentFilter =
    !raw || raw === "all" ? "all" : raw === "shared" ? "shared" : raw;

  const [employees, docs, faqs, metrics] = await Promise.all([
    listAiEmployees(organization.id),
    listKnowledgeDocuments(organization.id, { agentFilter }),
    listFaqs(organization.id, { agentFilter }),
    getKnowledgeMetrics(organization.id, { agentFilter }),
  ]);

  const agents = employees.map((e) => ({ id: e.id, name: e.name }));

  return (
    <div>
      <PageHeader
        title="Knowledge Base"
        description="Documents and FAQs shared across the org or assigned to a specific AI employee. Saving knowledge automatically updates that agent’s system prompt in AgentDesk and publishes it to Vapi."
        actions={
          <div className="flex flex-wrap items-start justify-end gap-2">
            <GenerateKnowledgePanel agents={agents} />
            <UploadDocumentForm agents={agents} />
          </div>
        }
      />

      <div className="mb-4">
        <Suspense fallback={null}>
          <KnowledgeAgentFilter agents={agents} />
        </Suspense>
      </div>

      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        <MetricCard label="Total Articles" value={metrics.totalArticles} />
        <MetricCard label="Published" value={metrics.published} />
        <MetricCard label="Drafts" value={metrics.drafts} />
        <MetricCard label="Processing" value={metrics.processing} />
        <MetricCard label="Failed" value={metrics.failed} />
      </div>

      <div className="mt-6 grid gap-4 xl:grid-cols-3">
        <div className="overflow-hidden rounded-xl border border-border bg-card shadow-sm xl:col-span-2">
          <div className="border-b border-border px-5 py-4">
            <h2 className="text-sm font-semibold text-foreground">Documents</h2>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full min-w-[800px] text-left text-sm">
              <thead className="border-b border-border bg-muted/40 text-muted-foreground">
                <tr>
                  <th className="px-4 py-3 font-medium">Article</th>
                  <th className="px-3 py-3 font-medium">Category</th>
                  <th className="px-3 py-3 font-medium">Assigned to</th>
                  <th className="px-3 py-3 font-medium">Status</th>
                  <th className="px-3 py-3 font-medium">Updated</th>
                  <th className="px-3 py-3 font-medium">Views</th>
                </tr>
              </thead>
              <tbody>
                {docs.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="px-4 py-8 text-center text-muted-foreground">
                      No documents for this filter.
                    </td>
                  </tr>
                ) : (
                  docs.map((doc) => (
                    <tr key={doc.id} className="border-b border-border/70">
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <FileText className="h-4 w-4 text-primary" aria-hidden />
                          <span className="font-medium text-foreground">{doc.title}</span>
                        </div>
                      </td>
                      <td className="px-3 py-3 text-muted-foreground">{doc.category ?? "—"}</td>
                      <td className="px-3 py-3 text-muted-foreground">
                        {assignedLabel(doc.agentId, doc.agentName)}
                      </td>
                      <td className="px-3 py-3">
                        <KnowledgeStatusBadge status={doc.status} />
                      </td>
                      <td className="px-3 py-3 text-muted-foreground">
                        {formatDate(doc.updatedAt, organization.timezone)}
                      </td>
                      <td className="px-3 py-3 text-muted-foreground">{doc.viewCount}</td>
                    </tr>
                  ))
                )}
              </tbody>
            </table>
          </div>
        </div>

        <div className="rounded-xl border border-border bg-card p-5 shadow-sm">
          <div className="flex items-center justify-between gap-2">
            <h2 className="text-sm font-semibold text-foreground">FAQs</h2>
            <CreateFaqForm agents={agents} />
          </div>
          <ul className="mt-4 space-y-4">
            {faqs.length === 0 ? (
              <li className="text-sm text-muted-foreground">No FAQs for this filter.</li>
            ) : (
              faqs.map((faq) => (
                <li key={faq.id} className="rounded-lg border border-border p-3">
                  <p className="text-sm font-medium text-foreground">{faq.question}</p>
                  <p className="mt-1 text-xs text-muted-foreground">{faq.answer}</p>
                  <div className="mt-2 flex flex-wrap gap-2 text-[11px] uppercase tracking-wide">
                    <span className="text-primary">{faq.category}</span>
                    <span className="text-muted-foreground">
                      {assignedLabel(faq.agentId, faq.agentName)}
                    </span>
                  </div>
                </li>
              ))
            )}
          </ul>
        </div>
      </div>
    </div>
  );
}
